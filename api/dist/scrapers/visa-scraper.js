...
export class VisaScraper extends BaseScraper {
    ...
    extractProcessingTime($section, _$) {
    ...
    `);
        stmt.run(item.country_code, item.visa_type, item.title, item.description, item.requirements, item.processing_time, item.cost, item.validity, item.source_url, item.source_name, item.language);
    }
}
export async function scrapeVisaForCountry(countryCode) {
    const scraper = new VisaScraper();
    return scraper.scrapeCountry(countryCode);
}
export async function scrapeAllVisaInfo() {
    const db = getDb();
    const countries = db
...
//# sourceMappingURL=visa-scraper.js.map