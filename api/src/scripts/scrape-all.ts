...
interface ScrapeResults {
  visa: ScrapeResult[];
  job: ScrapeResult[];
  housing: ScrapeResult[];
  healthcare: ScrapeResult[];
  banking: ScrapeResult[];
}
...
async function scrapeAll(): Promise<void> {
  ...
  console.log("Summary:");
  console.log(
    `  Visa:       ${results.visa.reduce((sum, r) => sum + r.count, 0)} entries`
  );
  console.log(
    `  Jobs:       ${results.job.reduce((sum, r) => sum + r.count, 0)} entries`
  );
  console.log(
    `  Housing:    ${results.housing.reduce((sum, r) => sum + r.count, 0)} entries`
  );
  console.log(
    `  Healthcare: ${results.healthcare.reduce((sum, r) => sum + r.count, 0)} entries`
  );
  console.log(
    `  Banking:    ${results.banking.reduce((sum, r) => sum + r.count, 0)} entries`
  );
}
