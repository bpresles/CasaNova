...
export class HousingScraper extends BaseScraper {
  ...
  async scrapeSource(source: Source, countryCode: string): Promise<HousingInfo[]> {
...
export async function scrapeHousingForCountry(
  countryCode: string
): Promise<HousingInfo[]> {
...
export async function scrapeAllHousingInfo(): Promise<ScrapeResult[]> {
  const db = getDb();
  const countries = db
