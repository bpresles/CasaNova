...
export class HealthcareScraper extends BaseScraper {
  ...
  async scrapeSource(source: Source, countryCode: string): Promise<HealthcareInfo[]> {
  ...
  saveHealthcareInfo(db: ReturnType<typeof getDb>, item: HealthcareInfo): void {
    ...
    stmt.run(
      ...
      item.title,
      item.description,
      item.public_system_info,
      item.insurance_requirements,
      item.emergency_numbers,
      item.useful_links,
      item.source_url,
      item.source_name,
      item.language
...
export async function scrapeHealthcareForCountry(
  countryCode: string
...
export async function scrapeAllHealthcareInfo(): Promise<ScrapeResult[]> {
  const db = getDb();
  const countries = db
